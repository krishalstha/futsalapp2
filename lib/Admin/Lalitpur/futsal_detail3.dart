...
class BookingDetailPage3 extends StatefulWidget {
  const BookingDetailPage3({Key? key}) : super(key: key);

  @override
  _BookingDetailPageState createState() => _BookingDetailPageState();
}

class _BookingDetailPageState extends State<BookingDetailPage3> {
  ...
  Future<void> acceptBooking(String bookingId, Map<String, dynamic> bookingData) async {
    try {
      await FirebaseFirestore.instance.collection('Lalitpurdetails').add(bookingData);
          ...
          actions: [
            TextButton(
              onPressed: () async {
                ...
                await FirebaseFirestore.instance.collection('bookingcort3').doc(bookingId).delete();
                setState(() {});
              },
              child: const Text('OK'),
            ),
